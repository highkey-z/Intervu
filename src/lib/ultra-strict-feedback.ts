export function getUltraStrictFeedback(question: string, answer: string): any {
  const answerLength = answer.length;
  const answerLower = answer.toLowerCase();
  
  // Detect terrible answers that should get very low scores
  const isTerribleAnswer = (
    // Repetitive nonsense patterns
    (answerLower.includes("blah") && (answerLower.split("blah").length - 1) >= 3) ||
    (answerLower.includes("test") && (answerLower.split("test").length - 1) >= 5) ||
    (answerLower.includes("hi") && (answerLower.split("hi").length - 1) >= 5) ||
    (answerLower.includes("hello") && (answerLower.split("hello").length - 1) >= 5) ||
    (answerLower.includes("yes") && (answerLower.split("yes").length - 1) >= 5) ||
    (answerLower.includes("no") && (answerLower.split("no").length - 1) >= 5) ||
    (answerLower.includes("ok") && (answerLower.split("ok").length - 1) >= 5) ||
    (answerLower.includes("yeah") && (answerLower.split("yeah").length - 1) >= 5) ||
    (answerLower.includes("sure") && (answerLower.split("sure").length - 1) >= 5) ||
    (answerLower.includes("maybe") && (answerLower.split("maybe").length - 1) >= 5) ||
    (answerLower.includes("idk") && (answerLower.split("idk").length - 1) >= 3) ||
    (answerLower.includes("idk") && (answerLower.split("idk").length - 1) >= 3) ||
    (answerLower.includes("whatever") && (answerLower.split("whatever").length - 1) >= 3) ||
    (answerLower.includes("nothing") && (answerLower.split("nothing").length - 1) >= 3) ||
    (answerLower.includes("something") && (answerLower.split("something").length - 1) >= 5) ||
    
    // Direct rejection patterns
    answerLower.includes("don't want this job") ||
    answerLower.includes("don't want the job") ||
    answerLower.includes("don't want your job") ||
    answerLower.includes("should not hire me") ||
    answerLower.includes("don't hire me") ||
    answerLower.includes("do not hire me") ||
    answerLower.includes("never hire me") ||
    answerLower.includes("useless") ||
    answerLower.includes("don't do anything") ||
    answerLower.includes("under any circumstances") ||
    answerLower.includes("ever again") ||
    answerLower.includes("leave me alone") ||
    answerLower.includes("never contact me") ||
    answerLower.includes("don't want to work here") ||
    answerLower.includes("never work here") ||
    
    // Interview negativity
    answerLower.includes("don't like") && answerLower.includes("interview") ||
    answerLower.includes("don't like") && answerLower.includes("you") ||
    answerLower.includes("want to go home") ||
    answerLower.includes("don't want to be") ||
    answerLower.includes("not interested") ||
    answerLower.includes("hate this") ||
    answerLower.includes("boring") ||
    answerLower.includes("stupid") ||
    answerLower.includes("waste of time") ||
    
    // General unprofessionalism
    answerLower.includes("don't") && answerLower.includes("important") ||
    answerLower.includes("not really important") ||
    answerLower.includes("just leave") ||
    answerLower.includes("don't care") ||
    answerLower.includes("doesn't matter") ||
    answerLower.includes("not my problem") ||
    answerLower.includes("not important") ||
    answerLower.includes("leave the issue") ||
    answerLower.includes("complexation") || // Grammatical error
    
    // Work pressure avoidance
    answerLower.includes("never have to work") ||
    answerLower.includes("don't want to work") ||
    answerLower.includes("avoid work") ||
    answerLower.includes("no pressure") ||
    
    // Self-deprecating unprofessional
    answerLower.includes("i'm useless") ||
    answerLower.includes("i'm not good") ||
    answerLower.includes("i can't do") ||
    answerLower.includes("i'm terrible") ||
    answerLower.includes("i'm bad at") ||
    
    // Length checks
    answerLength < 50 ||
    (answerLength < 100 && !answerLower.includes("i") && !answerLower.includes("my")) ||
    
    // Repetitive short answers
    (answerLength < 200 && (
      (answerLower.split(" ").length < 10) ||
      (answerLower.split(" ").filter(word => word.length > 2).length < 5)
    ))
  );
  
  // If it's a terrible answer, give very low scores
  if (isTerribleAnswer) {
    const feedback = [];
    
            // Check for repetitive nonsense patterns
            if ((answerLower.includes("blah") && (answerLower.split("blah").length - 1) >= 3) ||
                (answerLower.includes("test") && (answerLower.split("test").length - 1) >= 5) ||
                (answerLower.includes("hi") && (answerLower.split("hi").length - 1) >= 5) ||
                (answerLower.includes("hello") && (answerLower.split("hello").length - 1) >= 5) ||
                (answerLower.includes("yes") && (answerLower.split("yes").length - 1) >= 5) ||
                ...
                (answerLower.includes("nothing") && (answerLower.split("nothing").length - 1) >= 3) ||
                (answerLower.includes("something") && (answerLower.split("something").length - 1) >= 5)) {
              feedback.push("🚫 REPETITIVE NONSENSE: You repeated the same word/phrase multiple times instead of giving a proper answer. This shows complete lack of effort and professionalism.");
            }
            
            // Check for specific terrible patterns
            if (answerLower.includes("don't want this job") || answerLower.includes("don't want your job") || answerLower.includes("don't want to work here")) {
              feedback.push("🚫 JOB REJECTION: You explicitly stated you don't want the job. This is completely unprofessional in an interview.");
            }
    
    if (answerLower.includes("should not hire me") || answerLower.includes("don't hire me") || answerLower.includes("do not hire me")) {
      feedback.push("🚫 SELF-SABOTAGE: You told the interviewer not to hire you. This guarantees rejection.");
    }
    
    if (answerLower.includes("ever again") || answerLower.includes("never work here")) {
      feedback.push("🚫 ABSOLUTE REJECTION: You stated you never want to work here again. This is completely inappropriate.");
    }
    
    if (answerLower.includes("leave me alone") || answerLower.includes("never contact me")) {
      feedback.push("🚫 HOSTILE BEHAVIOR: Telling the interviewer to leave you alone is completely unprofessional and hostile.");
    }
    
    if (answerLower.includes("useless") || answerLower.includes("don't do anything")) {
      feedback.push("🚫 SELF-DEPRECATION: Calling yourself useless or saying you don't do anything is unprofessional and shows poor self-confidence.");
    }
    
    if (answerLower.includes("don't like") && (answerLower.includes("interview") || answerLower.includes("you"))) {
      feedback.push("🚫 INTERVIEW HOSTILITY: Expressing dislike for the interview or interviewer is completely inappropriate.");
    }
    
    if (answerLower.includes("want to go home") || answerLower.includes("don't want to be")) {
      feedback.push("🚫 UNPROFESSIONAL: Expressing desire to leave or not be present shows complete lack of interest.");
    }
    
    if (answerLower.includes("never have to work") || answerLower.includes("no pressure")) {
      feedback.push("🚫 WORK AVOIDANCE: Expressing desire to avoid work or pressure shows you're not suitable for employment.");
    }
    
    // Add general feedback if no specific patterns detected
    if (feedback.length === 0) {
      feedback.push("🚫 UNPROFESSIONAL: This answer demonstrates poor professional judgment and would result in immediate rejection.");
    }
    
    feedback.push("❌ CRITICAL: This response would result in immediate rejection from any professional interview.");
    feedback.push("❌ ATTITUDE PROBLEM: This demonstrates a fundamental misunderstanding of professional expectations.");
    ...
    return {
      ...
      confidence: 1,
      feedback,
      overall: "🚫 UNACCEPTABLE: This answer is completely unprofessional and would result in immediate rejection. You must completely rethink your approach to interviews and demonstrate proper professional judgment."
    };
  }
  
  // Regular scoring for non-terrible answers
