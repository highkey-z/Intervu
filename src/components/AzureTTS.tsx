...
import { generateAzureTTS, getAzureVoiceById, getAllAzureVoices } from '@/lib/azure-tts';
import AudioVisualizer from './AudioVisualizer';

interface AzureTTSProps {
  text: string;
  className?: string;
  autoSpeak?: boolean;
  onPlay?: () => void;
...
export default function AzureTTS({ text, className = '', autoSpeak = true, onPlay, onStop }: AzureTTSProps) {
  const [isPlaying, setIsPlaying] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedVoiceId, setSelectedVoiceId] = useState('en-US-AriaNeural'); // Default to Aria (most natural)
  ...
  useEffect(() => {
    // Check if Azure Speech API key is available
    setHasApiKey(!!process.env.NEXT_PUBLIC_AZURE_SPEECH_API_KEY);
  }, []);
  ...
  const generateAndPlayAudio = async () => {
    ...
    try {
      const voice = getAzureVoiceById(selectedVoiceId);
      if (!voice) {
      ...
      const result = await generateAzureTTS({
        text,
        ...
        volume: 2.0 // Slightly louder for better clarity
      });
      ...
      <div className={`flex items-center space-x-2 ${className}`}>
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-800">Azure TTS Not Available</p>
              <p className="text-xs text-yellow-700">Add AZURE_SPEECH_API_KEY to enable ultra-natural voices</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const selectedVoice = getAzureVoiceById(selectedVoiceId) || voices[0];
    ...
    <div className={`flex flex-col space-y-3 ${className}`}>
      ...
      {showVoiceSelector && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 space-y-2 max-h-80 overflow-y-auto">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">Powered by Azure Cognitive Services (Ultra-natural voices)</p>
