...
import { generateOpenAITTS, getOpenAIVoiceById, getAllOpenAIVoices } from '@/lib/openai-tts';
...
export default function OpenAITTS({ text, className = '', autoSpeak = true, onPlay, onStop }: OpenAITTSProps) {
  const [isPlaying, setIsPlaying] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedVoiceId, setSelectedVoiceId] = useState('alloy'); // Default to Alloy
  ...
  const voices = getAllOpenAIVoices();
      ...
      <div className={`flex items-center space-x-2 ${className}`}>
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-800">OpenAI TTS Not Available</p>
              <p className="text-xs text-yellow-700">Add OPENAI_API_KEY to enable ChatGPT-style voices</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const selectedVoice = getOpenAIVoiceById(selectedVoiceId) || voices[0];
    ...
    <div className={`flex flex-col space-y-3 ${className}`}>
      ...
      {showVoiceSelector && (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 space-y-2 max-h-80 overflow-y-auto">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">Powered by OpenAI (ChatGPT-style voices)</p>
